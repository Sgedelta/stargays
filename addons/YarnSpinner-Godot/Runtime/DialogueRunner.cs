...
/// <summary>
public void AdvanceAndSkip()
{
	// If the runner isn't active, do nothing.
	if (currentLineCancellationSource == null) return;

	// 1. Tell the UI to finish the typewriter effect immediately.
	currentLineHurryUpSource?.Cancel();
	
	// 2. Tell the DialogueRunner to move to the next line immediately.
	currentLineCancellationSource?.Cancel();
}
	/// <summary>
